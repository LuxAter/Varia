...
TEST(LuaTest, HasVar) {
  ...
  EXPECT_EQ(script->hasd("int_var"), true);
  EXPECT_EQ(script->hass("int_var"), true);
  ...
  EXPECT_EQ(script->hasvd("int_vec_var"), true);
  EXPECT_EQ(script->hasvs("int_vec_var"), true);
  EXPECT_EQ(script->hasf("int_vec_var"), false);

  EXPECT_EQ(script->has("float_var"), true);
  EXPECT_EQ(script->hasi("float_var"), true);
  EXPECT_EQ(script->hasd("float_var"), true);
  EXPECT_EQ(script->hass("float_var"), true);
  ...
  EXPECT_EQ(script->hasvi("float_vec_var"), true);
  EXPECT_EQ(script->hasvd("float_vec_var"), true);
  EXPECT_EQ(script->hasvs("float_vec_var"), true);
...
TEST(LuaTest, GetVar) {
  ...
  EXPECT_EQ(script->getd("int_var"), 2019.0);
  EXPECT_EQ(script->gets("int_var"), "2019");
  ...
  EXPECT_EQ(script->geti("float_var"), 3);
  EXPECT_EQ(script->getd("float_var"), 3.1415);
  EXPECT_EQ(script->gets("float_var"), "3.1415");
  ...
  EXPECT_EQ(script->getvd("int_vec_var"),
            std::vector<double>({10.0, 20.0, 30.0}));
  EXPECT_EQ(script->getvs("int_vec_var"),
  ...
  EXPECT_EQ(script->getvi("float_vec_var"), std::vector<int>({1, 1, 2}));
  EXPECT_EQ(script->getvd("float_vec_var"),
            std::vector<double>({1.0, 1.5, 2.0}));
  EXPECT_EQ(script->getvs("float_vec_var"),
            std::vector<std::string>({"1.0", "1.5", "2.0"}));

  EXPECT_EQ(script->geti("string_vec_var"), 0);
  EXPECT_EQ(script->getd("string_vec_var"), 0.0);
  EXPECT_EQ(script->gets("string_vec_var"), "");
  EXPECT_EQ(script->getvi("string_vec_var"), std::vector<int>({0, 0, 0}));
  EXPECT_EQ(script->getvd("string_vec_var"),
